...
To avoid reflecting user-controlled data, you should sanitize the `$pPage['title']` variable before using it in the HTML code. Here's how you can patch the code:
...
By using the `htmlspecialchars()` function, you convert special characters to their HTML entities. This prevents any user-controlled data from being interpreted as HTML, thereby mitigating the risk of Cross-Site Scripting (XSS) attacks. The `ENT_QUOTES` parameter ensures that both single and double quotes are properly encoded.
