...
To avoid constructing the path from user-controlled data, you can use a switch statement instead of directly referencing the ID in the file path. Here's the updated code:
...
In this updated code, the `getFilePath()` function is introduced to handle the construction of the file path based on the ID value. The function uses a switch statement to determine the correct file path for the given vulnerability ID. This approach ensures that the file path is not directly influenced by user-controlled data.
		<br />
