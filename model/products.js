...
By using the heredoc syntax (<<<HTML), we can avoid directly echoing out user-controlled data. Instead, we concatenate the HTML code as a string, which ensures that any user-controlled data is treated as plain text and not executed as code. This prevents potential security vulnerabilities like Cross-Site Scripting (XSS).
		<title>{$pPage[ 'title' ]}</title>
