...
if (array_key_exists ("id", $_GET)) {
    ...
    // Ensure the $id is a valid string without any directory traversal characters
    $id = preg_replace('/[^A-Za-z0-9_]/', '', $id);

    $lowsrc = @file_get_contents("./sources/{$id}/low.php");
    $lowsrc = str_replace( array( '$html .=' ), array( 'echo' ), $lowsrc);
    $lowsrc = highlight_string( $lowsrc, true );

    $medsrc = @file_get_contents("./sources/{$id}/medium.php");
    $medsrc = str_replace( array( '$html .=' ), array( 'echo' ), $medsrc);
    $medsrc = highlight_string( $medsrc, true );

    $highsrc = @file_get_contents("./sources/{$id}/high.php");
    $highsrc = str_replace( array( '$html .=' ), array( 'echo' ), $highsrc);
    $highsrc = highlight_string( $highsrc, true );

    $impsrc = @file_get_contents("./sources/{$id}/impossible.php");
