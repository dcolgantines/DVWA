...
To avoid constructing the path from user-controlled data, you should validate and sanitize the `$_GET['id']` value and use a whitelist approach to ensure only valid values are accepted. Here's the modified code:
...
$id = isset($_GET['id']) && in_array($_GET['id'], $validIds) ? $_GET['id'] : '';
...
In the modified code, we have added a `$validIds` array that contains the allowed values for the `id` parameter. We check if the `$_GET['id']` value exists in the whitelist using `in_array`. If it is a valid value, we HTML-encode it using `htmlspecialchars` and proceed with the rest of the code. If the value is not valid or not provided, the code will skip the vulnerable logic.
		case "xss_s" :
			$vuln = 'Stored XSS';
			break;
		case "weak_id" :
