...
$title = htmlspecialchars($pPage['title'], ENT_QUOTES, 'UTF-8');
echo "<title>{$title}</title>";
```
In this code, the variable `$pPage['title']` is user-controlled data that is directly inserted into the HTML output without any sanitization. This can lead to a cross-site scripting (XSS) vulnerability.

To patch this vulnerability, we can use the `htmlspecialchars` function to properly encode the user-controlled data before outputting it. This function converts special characters to their HTML entities, preventing any malicious scripts from being executed. We set the `ENT_QUOTES` flag to properly encode both single and double quotes, and specify the `UTF-8` encoding to handle special characters correctly.
