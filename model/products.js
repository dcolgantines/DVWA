...
To prevent reflecting user-controlled data, you should sanitize and escape the data before outputting it to the HTML. Here's an example of how you can achieve this:
...
$userData = "User's input <script>alert('XSS');</script>";

// Sanitize and escape the user data
...
In this example, we use the `htmlspecialchars` function to convert special characters to their corresponding HTML entities, preventing any active content (such as scripts) from being executed. This ensures that any user-controlled data is properly sanitized before outputting it to the HTML, mitigating the risk of cross-site scripting (XSS) attacks.
		<title>{$pPage[ 'title' ]}</title>
