...
To avoid reflecting user-controlled data directly in the code, we can use PHP's built-in function `htmlspecialchars` to properly encode the user input before outputting it. Here's the updated code:
...
Now, let's make sure that any user-controlled variables are being properly encoded using `htmlspecialchars`. Here's an example of how you can update the code to accomplish this:

```php
$userInput = $_GET['user_input']; // Assuming 'user_input' is the user-controlled variable
...
In this example, the `htmlspecialchars` function is used to properly encode the value of the `user_input` variable before it is echoed to the browser. This prevents any potential XSS attacks by encoding special characters that could be interpreted as HTML entities.
		<title>{$pPage[ 'title' ]}</title>
