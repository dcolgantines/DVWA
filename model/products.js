...
To avoid reflecting user-controlled data directly in the code, you can sanitize and validate the `title` variable before outputting it. Here's an example of how you can achieve that:
...
<head>
	...
	<title><?php echo htmlspecialchars($pPage['title']); ?></title>
</head>
<body>
...
In the updated code, we use the `htmlspecialchars()` function to sanitize the `title` variable. This function converts special characters to their corresponding HTML entities, preventing any potential HTML injection or XSS attacks.
