...
To avoid constructing the path from user-controlled data, you can use a whitelist to validate the `id` parameter. Here's the modified code:
...
if (array_key_exists("id", $_GET)) {
    ...
    if (in_array($id, $allowedIds)) {
        ...
        // Handle invalid id
        // For example, redirect back to a safe page
...
In the above code, `$allowedIds` is an array containing the whitelisted values for the `id` parameter. If the value of `id` is not present in the whitelist, you can handle it accordingly, such as by redirecting the user to a safe page.
		case "xss_s" :
			$vuln = 'Stored XSS';
			break;
		case "weak_id" :
