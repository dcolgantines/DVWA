...
To avoid reflecting user-controlled data, you should properly sanitize and escape any user input before using it in your HTML code. One way to achieve this is by using the htmlspecialchars function. Here's the modified code:
...
In this code, we use the `htmlspecialchars` function to escape any user input within the `$pPage['title']` variable. This prevents potential HTML injection attacks by converting special characters to their respective HTML entities. The `ENT_QUOTES` flag ensures that both single and double quotes are properly escaped.
