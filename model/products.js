...
if (array_key_exists ("id", $_GET)) {
    ...
    // Whitelist acceptable IDs to prevent path traversal
    $acceptableIds = array("javascript", "fi", "brute", "csrf", "exec", "sqli", "sqli_blind", "upload", "xss_r");
    if (in_array($id, $acceptableIds)) {
        $lowsrc = @file_get_contents(DVWA_WEB_PAGE_TO_ROOT . "dvwa/vulnerabilities/{$id}/source/low.php");
        $lowsrc = str_replace(array('$html .='), array('echo'), $lowsrc);
        $lowsrc = highlight_string($lowsrc, true);

        $medsrc = @file_get_contents(DVWA_WEB_PAGE_TO_ROOT . "dvwa/vulnerabilities/{$id}/source/medium.php");
        $medsrc = str_replace(array('$html .='), array('echo'), $medsrc);
        $medsrc = highlight_string($medsrc, true);

        $highsrc = @file_get_contents(DVWA_WEB_PAGE_TO_ROOT . "dvwa/vulnerabilities/{$id}/source/high.php");
        $highsrc = str_replace(array('$html .='), array('echo'), $highsrc);
        $highsrc = highlight_string($highsrc, true);

        $impsrc = @file_get_contents(DVWA_WEB_PAGE_TO_ROOT . "dvwa/vulnerabilities/{$id}/source/impossible.php");
