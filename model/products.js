...
To avoid constructing the path from user-controlled data, you can use a whitelist approach.
...
if (!in_array($id, $idWhitelist)) {
	$id = ""; // Reset id if not in the whitelist
}
...
if (!empty($id)) {
	...
	switch ($id) {
		...
		case "weak_id" :
			...
			break;
		case "authbypass" :
			$vuln = 'Authenticated Bypass';
			break;
	}
}
```

In the code above, we use an `$idWhitelist` array to store the allowed values for the `id` parameter. If the provided `id` is not in the whitelist, we reset it to an empty string.

This approach ensures that only valid values are used as paths, protecting against directory traversal attacks and unauthorized access to files.
