...
if (array_key_exists("id", $_GET)) {
    ...
    // Add a whitelist of allowed values for $id
    $allowedIds = ["javascript", "fi", "brute", "csrf", "exec", "sqli", "sqli_blind", "upload", "xss_r", "xss_s", "weak_id", "authbypass"];

    // Check if $id is in the allowedIds array
    if (in_array($id, $allowedIds)) {
        // Use a switch statement to handle each id individually
        switch ($id) {
            case "javascript":
                $vuln = 'JavaScript';
                break;
            ...
            case "weak_id":
                ...
                break;
            case "authbypass":
                // Handle "authbypass" case
                break;
        }

        // Only perform file operations if $id is not "authbypass"
        if ($id !== "authbypass") {
